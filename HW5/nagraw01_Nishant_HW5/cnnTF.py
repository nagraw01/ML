...
def complexModel():
    with tf.device(deviceType):
        ...
        wConv = tf.get_variable("wConv", shape=[7, 7, 3, 32])
        bConv = tf.get_variable("bConv", shape=[32])
        wmid = tf.get_variable("wmid", shape=[1152, 1024])  # Stride = 2, ((32-7)/2)+1 = 13, 13*13*32=5408
        bmid = tf.get_variable("bmid", shape=[1024])
        w = tf.get_variable("w", shape=[1024, 10])  # Stride = 2, ((32-7)/2)+1 = 13, 13*13*32=5408
        b = tf.get_variable("b", shape=[10])

        # Define Convolutional Neural Network
        a = tf.nn.conv2d(x, wConv, strides=[1, 2, 2, 1],
                         padding='VALID') + bConv  # Stride [batch, height, width, channels]

        h = tf.nn.relu(a)
        mp = tf.nn.max_pool(h, [1, 2, 2, 1], [1, 2, 2, 1], padding='VALID')# (13-2)/2+1 = 6, 6*6*32=1152

        hFlat = tf.reshape(mp, [-1, 1152])

        ymid = tf.matmul(hFlat, wmid) + bmid
        hmid = tf.nn.relu(ymid)
        yOut = tf.matmul(hmid, w) + b
        pass
        #########################################################################
        #                       END OF YOUR CODE                                #
        #########################################################################
...
def yourOwnModel():
    with tf.device(deviceType):
        ...
        wConv = tf.get_variable("wConv", shape=[5, 5, 3, 32])
        bConv = tf.get_variable("bConv", shape=[32])
        wmid = tf.get_variable("wmid", shape=[1568, 1024])  # Stride = 2, ((32-5)/2)+1 = 14, 14*14*32, (14-2)/2+1 = 7, 7*7*32=1568
        bmid = tf.get_variable("bmid", shape=[1024])
        wmid2 = tf.get_variable("wmid2", shape=[1024, 300])
        bmid2 = tf.get_variable("bmid2", shape=[300])
        wmid3 = tf.get_variable("wmid3", shape=[300, 100])
        bmid3 = tf.get_variable("bmid3", shape=[100])
        # wmid4 = tf.get_variable("wmid4", shape=[256, 128])
        # bmid4 = tf.get_variable("bmid4", shape=[128])
        ...
        h = tf.nn.relu(a)
        mp = tf.nn.max_pool(h, [1, 2, 2, 1], [1, 2, 2, 1],
                            padding='VALID')

        hFlat = tf.reshape(mp, [-1, 1568])

        ymid = tf.matmul(hFlat, wmid) + bmid
        hmid = tf.nn.relu(ymid)
        ymid2 = tf.matmul(hmid, wmid2) + bmid2
        hmid2 = tf.nn.relu(ymid2)
        ymid3 = tf.matmul(hmid2, wmid3) + bmid3
        hmid3 = tf.nn.relu(ymid3)
        # ymid4 = tf.matmul(hmid3, wmid4) + bmid4
        # hmid4 = tf.nn.relu(ymid4)
        yOut = tf.matmul(hmid3, w) + b
...
#########################################################################
train(yourOwnModel(), xTrain, yTrain, xVal, yVal, xTest, yTest, batchSize = 500, epochs = 200)
#########################################################################
#                       END OF YOUR CODE                                #
#########################################################################
