...
################################################################################
pass
learningRate = [8e-8, 1e-7, 5e-6, 5e-3]
regularizationStrength = [5e-3, 7e1, 6e2, 4e3]
numClasses = np.max(yTrain) + 1
hiddenNeurons = 100

iters = 1500 #100
for lrt in learningRate:
    for rs in regularizationStrength:
        classifier = TwoLayersNN(xTrain.shape[1], hiddenNeurons, numClasses)
        classifier.train(xTrain, yTrain, lr=lrt, reg=rs, iterations=iters, verbose=True)
        accCal = classifier.calAccuracy(xVal, yVal)
