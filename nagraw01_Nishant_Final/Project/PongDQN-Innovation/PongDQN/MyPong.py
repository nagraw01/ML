...
#draw our ball
def drawBall(Ball_X_pos, ballYPos, BallCol):
    #small rectangle, create it
    ball = pygame.Rect(Ball_X_pos, ballYPos, BALL_WIDTH, BALL_HEIGHT)
    #draw it
...
def updateBall(paddle1YPos, paddle2YPos, Ball_X_pos, ballYPos, Ball_X_Dir, Ball_Y_Dir,dft,BallColour):
	dft =7.5
	#update the x and y position
	Ball_X_pos = Ball_X_pos + Ball_X_Dir * BALL_X_SPEED*dft
	ballYPos = ballYPos + Ball_Y_Dir * BALL_Y_SPEED*dft
	score = 0
	NewBallColor = BallColour; 
    #checks for a collision, if the ball hits the Gamer Player side, our Learning agent
	if (Ball_X_pos <= PADDLE_BUFFER + PADDLE_WIDTH and ballYPos + BALL_HEIGHT >= paddle1YPos and ballYPos - BALL_HEIGHT <= paddle1YPos + PADDLE_HEIGHT and Ball_X_Dir == -1):
		#switches directions
		Ball_X_Dir = 1
	...
	# Check if Ball past Player
		...
		score = -10.0
		NewBallColor = RED
		return [score, Ball_X_pos, ballYPos, Ball_X_Dir, Ball_Y_Dir,NewBallColor]

	#check if hits the AI Player
	if (Ball_X_pos >= WINDOW_WIDTH - PADDLE_WIDTH - PADDLE_BUFFER and ballYPos + BALL_HEIGHT >= paddle2YPos and ballYPos - BALL_HEIGHT <= paddle2YPos + PADDLE_HEIGHT):
		#switch directions
		Ball_X_Dir = -1
		NewBallColor = WHITE
	#past it
...
class PongGame:
	...
	def InitialDisplay(self):
		...
		#draw our ball
		drawBall(self.Ball_X_pos, self.ballYPos,WHITE)
		#
		#updates the window
		pygame.display.flip()
	...
	def PlayNextMove(self, action):
		...
		[score, self.Ball_X_pos, self.ballYPos, self.Ball_X_Dir, self.Ball_Y_Dir,self.BallColor] = updateBall(self.paddle1YPos, self.paddle2YPos, self.Ball_X_pos, self.ballYPos, self.Ball_X_Dir, self.Ball_Y_Dir,DeltaFrameTime,self.BallColor)
		#draw the ball
		drawBall(self.Ball_X_pos, self.ballYPos,self.BallColor)
		#
		...
		return [score,self.paddle1YPos, self.Ball_X_pos, self.ballYPos, self.Ball_X_Dir, self.Ball_Y_Dir]

	# Return the Curent Game State
	def ReturnCurrentState(self):
		# Simply return state
		score = 0
		return [self.paddle1YPos, self.Ball_X_pos, self.ballYPos, self.Ball_X_Dir, self.Ball_Y_Dir]
